 ...
 */
class ConfigConnectionDataSectionsAdapter(val context: Context, fm: FragmentManager): FragmentPagerAdapter(fm) {
    ...
    override fun getPageTitle(position: Int) = context.getString(when(position) {
        EMAIL_PAGE_POS -> {
            ConfigEmailFragment.TAB_TITLE
        } else -> {
            ConfigPasswordFragment.TAB_TITLE
        }
    })
